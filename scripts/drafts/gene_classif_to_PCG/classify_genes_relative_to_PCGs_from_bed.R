...
query_data_path <- args[1] # path to query bed file

ref_gtf_path="data/references/merged_refs_annotation/merged_refs.combined.with_gene_name.gtf"
gene_biot_path="data/references/merged_refs_annotation/annotated_tracking_file.updated_gene_names.txt"
...
get_gene_class <- function(ref_gtf,query_bed,N=T,k=4){
  require(bedtoolsr)
  colnames(query_bed) <- c("seqnames","start","end","gene_name","width","strand")
  ...
  ord_chrs <- as.character(unique(ref_bed$seqnames))
  query_bed <- query_bed%>%dplyr::arrange(match(seqnames,ord_chrs),start)
...
gene_biot <- read.table(gene_biot_path,header = T)

# query data
query_bed <- read.table(query_data_path,header = F)
...
class_summary=summarise_classif(closest_class = closest_class_to_PCG)
...
# add info to tables ----
query_bed <- left_join(query_bed, class_summary)

# add distance to closest gene
query_target_id <- paste0(query_bed$gene_name,";",
                          query_bed$best_closest_PCG)
query_target_id_ol <- paste0(closest_class_to_PCG$idQ,
                             ";",
                             closest_class_to_PCG$idT)
query_bed$dist_closest_PCG <- abs(closest_class_to_PCG$dist[match(query_target_id,
                                                                  query_target_id_ol)])
# write results ----
out_path_closest_class <- paste0("outputs/",
                                 gsub(".bed",".closest_PCGs.txt",basename(query_data_path)))
write.table(closest_class_to_PCG,out_path_closest_class,quote = F,row.names = F,sep = "\t")

out_path_transcript_level=gsub(".bed",".gene_classif.tsv",query_data_path)
write.table(query_bed,out_path_transcript_level,quote = F,row.names = F,sep = "\t")
