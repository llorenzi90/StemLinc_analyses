...
# The individual assemblies from all samples were combined and compared with gffCompare and against the
...
class_order <- names(sort(table(tracking_biots_2samps$transfrag_biotype),decreasing = T))

N_libsGL <- get_n_samples_per_gene_from_tracking(tracking_biots_2samps %>% mutate(V2=gene_id),
                                                 cols = 6:14)

cols = 6:14
sample_occurrence_per_gene = lapply(cols, function(sa) {
  stats::aggregate(tracking_biots_2samps[, sa], by = list(gene_id = tracking_biots_2samps$gene_id),
                   function(x) any(x != "-"))
})
...
summarise_at_gene_level <- function(ftracking,cols=6:14){
  ...
  N_libsGL <- get_n_samples_per_gene_from_tracking(ftracking %>% mutate(V2=gene_id),
                                                   cols = 6:14)
...
# remove all new genes that are only found in one library ----
filtered_tracking <- filtered_tracking %>% mutate(gffGeneid=V2,
                                                  V2=ifelse(overlapRef,ref_gene_id,
                                                            V2),
                                                  gene_id=V2)

filtered_tracking

colnames(filtered_tracking)
filtered_genes_N_libraries <- get_n_samples_per_gene_from_tracking(filtered_tracking,cols = 6:14)
table(filtered_genes_N_libraries)

filtered_tracking <- filtered_tracking %>%
  filter(V2%in%names(filtered_genes_N_libraries)[filtered_genes_N_libraries>1])
...
assignment_df$gene_id <- multiexons_bed$V2[match(assignment_df$transcript_id,multiexons_bed$V1)]

assignment_df <- assignment_df %>%filter(gene_id==gene_cluster_id)
assignment_df$cluster_id <- paste(assignment_df$cluster_start,assignment_df$cluster_end,
                                  sep = "_")
